...
The transmission of each variant is modeled using a deterministic renewal equation that allows for realistic delay distributions between infection, transmission, and detection as a case.
...
\begin{figure}[h!]
  ...
  \caption[\textbf{Fitting the fixed growth advantage model to Washington state data.}]{\textbf{Fitting the fixed growth advantage model to Washington state data.}
...
\begin{figure}[h!]
  ...
  \caption[\textbf{Fitting the GARW model to Washington state data.}]{\textbf{Fitting the GARW model to Washington state data.}
...
\begin{figure}[h!]
  ...
  \caption[\textbf{Inferred effective reproduction numbers from GARW model in 34 states show consistent trends of variants across states.}]{\textbf{Inferred effective reproduction numbers from GARW model in 34 states show consistent trends of variants across states.}
...
\begin{figure}[h!]
  ...
  \caption[\textbf{Using fixed growth advantage model, we infer growth advantages for 8 variants in 34 US states.}]{\textbf{Using fixed growth advantage model, we infer growth advantages for 8 variants in 34 US states.}
...
\begin{figure}[h!]
  ...
  \caption[\textbf{Estimating variant growth advantages in 34 states using GARW model.}]{\textbf{Estimating variant growth advantages in 34 states using GARW model.}
...
\begin{figure}[h!]
  ...
  \caption[\textbf{Estimating growth advantages of Omicron sublineages relative to BA.1 in 33 US states.}]{\textbf{Estimating growth advantages of Omicron sublineages relative to BA.1 in 33 US states.}
...
Further, as case surveillance for COVID-19 has decreased in reliability after 2022, we note that this method is still applicable using other proxies for infection incidence such as hospitalization data or wastewater testing.
...
\end{equation}
where $[t] = t \mod 7 + 1$, $\alpha$ is an over-dispersion parameter relative to the Poisson distribution, and $\text{NegBinom}(\mu, \alpha)$ is the negative binomial distribution with mean $\mu$ and variance  $\mu + \alpha\mu^{2}$.
In the case of multiple variants, we use $P(t) = \sum_{1\leq v \leq V} P_{v}(t)$.
...
If we suppose that we are tracking the growth of $V$ variants, our data for a given day $t$ takes the form of daily counts $C_{t} = (C_{t,1}, \ldots, C_{t,V})$ of sequences of each variant with daily total $N_{t} = \sum_{1\leq v \leq V} C_{t, v}$.
...
% For the purposes of the wave size analysis in Figure \ref{fig:intro_R_to_wave_size}, we define the first successful introduction to be the first time point at which variant $v$ is above a threshold frequency of 5\% and have effective reproduction number greater than one.

...
Further, these methods have shown difficulty scaling to large numbers of sequences ($>$ 100,000) and can lack the flexibility to integrate diverse data sources, employ customizable models, or generate interpretable visualizations. %TODO: Cite
...
% TODO: Mention that models can have multiple backends though most methods are implemented within numpyro
...
% \forecastsNcov\ enhances Nextstrain’s phylogenetic tools by integrating real-time forecasting with phylogenetic data, enabling a comprehensive understanding of how variants evolve over time and how they will continue to spread.
% This integration allows for both historical insights (via phylogenetic trees) and future predictions (via growth advantage estimates), providing a full picture of viral evolution that is critical for public health planning.
% By combining these aspects, \forecastsNcov\ helps shift the focus from just visualizing genetic diversity to actively forecasting the future trajectory of variants.
% Real-Time Forecasting as a Public Health Tool: The platform is invaluable for real-time applications such as identifying emerging variants of concern and predicting potential waves of infection. The forward-looking predictions provided by \forecastsNcov\ directly support policy decisions regarding resource allocation and vaccination strategy.
% It enhances Nextstrain’s real-time utility, allowing public health experts to anticipate the emergence of variants rather than just track them after the fact.
% Archival Nature of Forecasts: \forecastsNcov’s ability to archive forecasts and model assumptions adds value beyond real-time decision-making. These archives serve as a record of evolutionary forecasting, providing a resource for retrospective analysis and methodological refinement.
%
% Public health officials and researchers can use archived forecasts to evaluate the accuracy of predictions, test new forecasting methodologies, and refine the models used in future outbreaks.
% Scientific Advancement: By integrating real-time forecasting with phylogenetic analysis, \forecastsNcov\ supports the advancement of evolutionary forecasting science. It enables researchers to test evolutionary hypotheses, evaluate model performance, and improve forecasting accuracy over time.
% Scientific and Public Health Impact: 
%
% Public Health Preparedness: \forecastsNcov\ positions itself as a critical tool in pandemic preparedness, offering predictions that help guide interventions while providing a historical record that can inform future responses.
% Future Directions:
% Expansion to Other Pathogens: \forecastsNcov\ can be extended to other pathogens, such as influenza or bacterial infections, with modifications to handle different genomic structures and epidemiological dynamics.
% Data Stream Integration:
% Integrate additional data streams, such as behavioral data or environmental factors, to improve forecasting capabilities beyond genomic and case data.
% Enhanced Visualization and User Access:
% Further develop user-friendly interfaces and interactive tools to make forecasting results more accessible to non-expert users and public health officials.
%
% Archival Value Emphasized: The discussion of how \forecastsNcov\ archives forecasts and model assumptions provides significant value, especially for retrospective analysis, refinement of methodologies, and fostering transparency in public health forecasting.
% Broader Pathogen Applications: The outline now highlights the platform’s adaptability and potential for use with other pathogens, expanding the applicability of \forecastsNcov\ to global health challenges beyond SARS-CoV-2.
% Integration with Real-Time Public Health Efforts: It clearly links the platform’s outputs to actionable public health strategies, helping to bridge the gap between data-driven forecasts and real-world decision-making.
%

%TODO: Read through this thoroughly and make sure it fits the vision
% Incorporate ideas from above
...
%TODO: Many of these are really contributions. We should use discussion to talk about the potential here.

...
%TODO: Add discussion on recurrent waves
Following the end of the pandemic, there have been multiple infection waves caused by seasonality in transmission as well as evolution of novel SARS-CoV-2 variants which can evade existing immune responses in individuals with past exposure.

%TODO: Cite various papers mentioned throughout on individual variant waves.
...
Evolution in SARS-CoV-2 has produced genetic variants which exhibit strain-specific immune responses in humans as measured through serology, neutralization titers, and deep mutational scanning assays. %TODO: Cite here
...
As SARS-CoV-2 continues to transmit, the virus will mutate and new variants with unique traits emerge, influencing their transmissibility, severity, and ability to evade immunity.
...
Accurate models are needed to predict how these variants will evolve and spread, helping to monitor SARS-CoV-2 evolution and mitigate future outbreaks.
...
We will then discuss existing methods for understanding population turnonver from population genetics, their relationship to some of the related quantities in the epidemiological context.
...
In order for evolution to occur, we need a counter-balance of mutation (generating heritable diversity) and selection (differential survival and reproduction based on that diversity).
...
This gives us the following system of ordinary differential equations:
\begin{align}
...
As viruses transmit between individuals, they generate genetic variation through mutation, drift, and selection.
...
Selection acts as a ``force'' on the genetic diversity of the population favoring particular genetic variants causing the population to evolve in response to particular environmental pressures.

In short,  we can define selection and evolution as:
...
Using this, we can write the expected frequency of allele $A$, $p_A = \frac{x_A}{x_A + x_B}$, and its variance as
...
%TODO: Note that these are not the only forces of population turnover and discuss drift and effective population size briefly.

%TODO: Specify Haploid population throughout!!
To measure the magnitude of selection for a particular allele, we can define the selective coefficient $s$ so that the fitness of $A$ is $W_A = (1+s) W_B$ per generation.
...
% TODO: Update this to exponential growth framing to talk about how selective advantage relates to difference in exponential growth rates
...
This provides a simple relationship between the traditional selective coefficient $s$ used in population genetics and the relative fitness $\lambda$ that we'll use throughout this dissertation.
...
In practice, we want may want to additionally evolutionarily meaningful clades that also minimize phenotype variance.
...
This gives us the following approximation for the expected phenotype across the population:

\begin{equation}
  \Expect_{X_t}[G] \approx \sum_{u} f_{u}(t) \overline{G}_u,
\end{equation}

which reduces a potentially complex integral into a weighted sum of variant means.
...
We can see this by developing a bound on the error of this approximation
...
\end{align}
which depends on the intrinsic variance with the variant and the squared bias in the estimated variant mean.
...
This shows our approximation is exhibits a bias-variance trade-off, but generally for unbiased estimates will be most accurate when the within-variant variance is minimized, the variants are well-separated in the phenotypic space.
If the variance of the phenotype within a variant is too large, the mean phenotype within the variant may no longer represent all sequences within the variant.
...
We'll begin by describing the major targets of evolutionary forecasting mathematically using models of frequency change.
...
When quantifying genetic diversity in practice, we typically work with variant classification and the frequencies of these variants as summaries of this genetic diversity. 
...
\begin{itemize}
  ...
  % \item Validation of the framework’s predictive accuracy and scalability, highlighting its potential for real-time monitoring and broader application to other rapidly evolving pathogens.
\end{itemize}
